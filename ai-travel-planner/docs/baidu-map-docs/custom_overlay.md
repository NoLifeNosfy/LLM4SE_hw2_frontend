...
自定义覆盖物组件是一个高度订制化的组件，通过 draw 事件进行覆盖物的重绘。推荐的使用方法是将重绘逻辑相同的 BmOverlay 进行二次封装。自定义覆盖物仅在地图发生变化时触发 draw 方法重绘覆盖物视图，若需要定制数据视图同步策略，请手动触发 BmOverlay 的 reload 实例方法。

## 属性

| 属性名 | 类型 | 默认值 | 描述 |
|--------|------|--------|------|
| pane | String |  | 自定义覆盖物所在容器。包含 floatPane、markerMouseTarget、floatShadow、labelPane、markerPane、markerShadow、mapPane 共 7 种合法值 |
