<template>
  <div class="planner-container">
    ...
    <div class="toolbar">
        ...
        <el-button 
            :type="selectedTrips.length > 0 ? 'danger' : ''"
            :icon="Delete"
            :disabled="selectedTrips.length === 0"
            @click="handleDeleteTrips"
        >Delete Trips</el-button>
    </div>
    <el-card v-for="trip in trips" :key="trip.id" class="trip-card">
      <template #header>
        <div class="card-header">
          <span>{{ trip.title }}</span>
          <el-checkbox v-model="selectedTrips" :value="trip.id" size="large" class="delete-checkbox" />
        </div>
      </template>
      <div>
...
const selectedTrips = ref<string[]>([]);
...
const handleDeleteTrips = async () => {
  if (selectedTrips.value.length === 0) {
    ElMessage.warning('Please select at least one trip to delete.');
    return;
  }

  try {
    await ElMessageBox.confirm(
      `Are you sure you want to delete the following trips?\n\n${selectedTrips.value.map(id => trips.value.find(trip => trip.id === id)?.title).join('\n')}`,
      'Confirm Deletion',
      {
    ...
    await tripStore.deleteTrips(selectedTrips.value);
    ElMessage.success('Trips deleted successfully.');
    selectedTrips.value = [];
...
.delete-checkbox .el-checkbox__inner {
  background-color: #f08080; /* Gray-red color */
  border-color: #f08080;
}
